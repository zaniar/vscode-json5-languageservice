...
class SchemaHandle implements ISchemaHandle {
	...
	private service: JSON5SchemaService;

	constructor(service: JSON5SchemaService, url: string, unresolvedSchemaContent?: JSON5Schema) {
...
export class ResolvedSchema {
	...
	public getSection(path: string[]): JSON5Schema | undefined {
	...
	private getSectionRecursive(path: string[], schema: JSON5SchemaRef): JSON5SchemaRef | undefined {
...
export class JSON5SchemaService implements IJSON5SchemaService {
	...
	private addSchemaHandle(id: string, unresolvedSchemaContent?: JSON5Schema): SchemaHandle {
		const schemaHandle = new SchemaHandle(this, id, unresolvedSchemaContent);
		this.schemasById[id] = schemaHandle;
		return schemaHandle;
	}

	private getOrAddSchemaHandle(id: string, unresolvedSchemaContent?: JSON5Schema): SchemaHandle {
	...
	public registerExternalSchema(uri: string, filePatterns?: string[], unresolvedSchemaContent?: JSON5Schema): ISchemaHandle {
	...
	public loadSchema(url: string): Thenable<UnresolvedSchema> {
		if (!this.requestService) {
			const errorMessage = localize('json5.schema.norequestservice', 'Unable to load schema from \'{0}\'. No schema request service available', toDisplayString(url));
			return this.promise.resolve(new UnresolvedSchema(<JSON5Schema>{}, [errorMessage]));
		}
		return this.requestService(url).then(
			content => {
				if (!content) {
					const errorMessage = localize('json5.schema.nocontent', 'Unable to load schema from \'{0}\': No content.', toDisplayString(url));
					return new UnresolvedSchema(<JSON5Schema>{}, [errorMessage]);
				}

				let schemaContent: JSON5Schema = {};
				const jsonErrors: Json.ParseError[] = [];
				schemaContent = Json.parse(content, jsonErrors);
				const errors = jsonErrors.length ? [localize('json5.schema.invalidFormat', 'Unable to parse content from \'{0}\': Parse error at offset {1}.', toDisplayString(url), jsonErrors[0].offset)] : [];
			...
			(error: any) => {
				...
				return new UnresolvedSchema(<JSON5Schema>{}, [localize('json5.schema.nocontent', 'Unable to load schema from \'{0}\': {1}.', toDisplayString(url), errorMessage)]);
			}
		);
	}
	...
	public resolveSchemaContent(schemaToResolve: UnresolvedSchema, schemaURL: string, dependencies: SchemaDependencies): Thenable<ResolvedSchema> {
		...
		if (schema.$schema) {
			...
			if (id === 'http://json-schema.org/draft-03/schema') {
				...
				schemaToResolve.errors.push(localize('json5.schema.draft201909.notsupported', "Draft 2019-09 schemas are not yet fully supported."));
			}
		}

		const contextService = this.contextService;

		const findSection = (schema: JSON5Schema, path: string | undefined): any => {
			if (!path) {
				return schema;
			}
		...
		const merge = (target: JSON5Schema, sourceRoot: JSON5Schema, sourceURI: string, refSegment: string | undefined): void => {
			...
			if (section) {
				...
				resolveErrors.push(localize('json5.schema.invalidref', '$ref \'{0}\' in \'{1}\' can not be resolved.', path, sourceURI));
			}
		};

		const resolveExternalLink = (node: JSON5Schema, uri: string, refSegment: string | undefined, parentSchemaURL: string, parentSchemaDependencies: SchemaDependencies): Thenable<any> => {
			...
			return referencedHandle.getUnresolvedSchema().then(unresolvedSchema => {
				...
				if (unresolvedSchema.errors.length) {
					...
					resolveErrors.push(localize('json5.schema.problemloadingref', 'Problems loading reference \'{0}\': {1}', loc, unresolvedSchema.errors[0]));
				}
		...
		const resolveRefs = (node: JSON5Schema, parentSchema: JSON5Schema, parentSchemaURL: string, parentSchemaDependencies: SchemaDependencies): Thenable<any> => {
			if (!node || typeof node !== 'object') {
				return Promise.resolve(null);
			}

			const toWalk: JSON5Schema[] = [node];
			const seen: JSON5Schema[] = [];

			const openPromises: Thenable<any>[] = [];

			const collectEntries = (...entries: (JSON5SchemaRef | undefined)[]) => {
			...
			const collectMapEntries = (...maps: (JSON5SchemaMap | undefined)[]) => {
				for (const map of maps) {
					if (typeof map === 'object') {
						for (const k in map) {
							const key = k as keyof JSON5SchemaMap;
							const entry = map[key];
							if (typeof entry === 'object') {
								toWalk.push(entry);
							}
			...
			const collectArrayEntries = (...arrays: (JSON5SchemaRef[] | undefined)[]) => {
			...
			const handleRef = (next: JSON5Schema) => {
				const seenRefs = [];
				while (next.$ref) {
					const ref = next.$ref;
				...
				collectEntries(<JSON5Schema>next.items, <JSON5Schema>next.additionalProperties, next.not, next.contains, next.propertyNames, next.if, next.then, next.else);
				collectMapEntries(next.definitions, next.properties, next.patternProperties, <JSON5SchemaMap>next.dependencies);
				collectArrayEntries(next.anyOf, next.allOf, next.oneOf, <JSON5Schema[]>next.items);
			};
	...
	public getSchemaForResource(resource: string, document?: Parser.JSON5Document): Thenable<ResolvedSchema | undefined> {
	...
	private createCombinedSchema(resource: string, schemaIds: string[]): ISchemaHandle {
		if (schemaIds.length === 1) {
			...
			const combinedSchema: JSON5Schema = {
